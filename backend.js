...
process.env.EMAIL_USER = 'l20211953@tectijuana.edu.mx';
...
function classifyAudioWithPython(audioPath) {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(__dirname, 'python', 'classify.py');
    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        ...
        try {
          ...
          reject('Salida inválida de Python: ' + stdout);
        }
      }
    );
  });
...
app.post('/api/analizar-audio', upload.single('audio'), verifyTokenMiddleware, async (req, res) => {
  try {
    ...
    try {
      const command =
        `"${ffmpegPath}" -y -i "${req.file.path}" ` +
        `-ac 1 -ar 2000 -acodec pcm_f32le -f f32le "${tempFile}" ` +
        `2> "${logFile}"`;

      execSync(command, { stdio: 'inherit' });

      if (!fs.existsSync(tempFile)) {
        throw new Error('FFmpeg no generó muestras');
      }
      ...
      const normalizedSamples = Array.from(samples).map(s => s / (max || 1));

      // 🔥 CLASIFICACIÓN CON PYTHON 🔥
      const classification = await classifyAudioWithPython(req.file.path);
      ...
      res.json({
        success: true,
        message: 'Audio analizado y clasificado exitosamente',
        audioAnalysis: {
...
/*
// ANALIZAR AUDIO Y EXTRAER MUESTRAS REALES (PREVIO)
app.post('/api/analizar-audio', upload.single('audio'), verifyTokenMiddleware, async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ success: false, message: 'Archivo no proporcionado' });
